...
function formatDate(date, formats, timezoneOffset) {
    ...
    formats = formats.replace('MM', (dt.getMonth() + 1 > 9
        ? dt.getMonth() + 1
...
function isSameOrBefore(input, date) {
    const sam = isSame(input, date) || isBefore(input, date);
    //console.log(sam);
    return sam;
...
class HelloWeek {
            ...
            if (!range) {
                if (multiplePick) {
                    if (this.days[index].date) {
                        this.daysSelected = this.daysSelected.filter((day) => formatDateToCompare(day) !==
    ...
    createDay(date) {
        ...
        if (locked ||
            ...
            (disablePastDays && isSameOrBefore(this.date, this.todayDate)) ||
    ...
    disabledDays(dayOptions) {
        ...
        if (isArray(disableDates[0])) {
            disableDates.map((date) => {
                if (isSameOrAfter(dayOptions.date, date[0]) &&
